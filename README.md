...
For protein function prediction (PFP), since the raw data is very large (around 2125 * 2GB), we uploaded 5 of the GO terms which have been transformed into the format for EI. The remaining terms can be generated by the STRING DB (`PFP/STRING_csv`) & GO annotation files (`GO_annotation.tsv`) using `generate_data.py`
...
Arguments of train_base.py:
	...
	--queue, -Q: LSF queue to submit the job (Minerva only)
	--node, -N: number of node requested to HPC
	--time, -T: number of hours requested to HPC
	--memory, -M: memory requsted in MB to HPC
	--hpc: use HPC cluster or general use GNU parallel implementation
	--fold, -F: number of cross-validation (outer) folds
	--rank: Boolean to use local feature ranking or not (default:False)

Option 1: Without access to HPC, EI can be run sequentially.

	python train_base.py --path [path] --hpc False

Option 2: Run the pipeline in parallel using the GNU parallel implementation.

	python train_base.py --path [path] --hpc parallel

Option 3: Run the pipeline in parallel on Minerva.
...
| Support Vector Machine | sklearn.svm.SVC(kernel='linear', probability=True) |
