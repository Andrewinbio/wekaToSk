...
def fmeasure_score(labels, predictions, thres=None, beta=1.0, pos_label=1):
    ...
    if thres is None:
        ...
        opt_threshold = threshold[arg_max] if arg_max < len(threshold) else 1.0
        fmax = f1[arg_max]
        pmax = precision[arg_max]  # precision at which f is maximised
        rmax = recall[arg_max]  # recall at which f is maximised
        # fmax = nanmax(f1)
...
def data_dir_list(data_path, excluding_folder=['analysis', 'feature_rank', '__pycache__']):
